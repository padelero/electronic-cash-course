...
export function Navbar() {
  ...
  return <nav className="bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-50 w-full border-b">
        ...
        <div className="flex justify-between h-16">
          <div className="flex">
            <Link to="/" className="flex-shrink-0 flex items-center">
              ...
              <span className="ml-2 font-montserrat font-bold text-xl">Electronic Cash Course</span>
            </Link>
            <div className="hidden sm:ml-6 sm:flex sm:space-x-8">
              <Link to="/courses" className="inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium leading-5 hover:border-bitcoin hover:text-foreground transition">
                Cursos
              </Link>
          ...
          <div className="-mr-2 flex items-center sm:hidden">
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} type="button" className="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-bitcoin" aria-controls="mobile-menu" aria-expanded="false">
              <span className="sr-only">Abrir menú principal</span>
              {isMobileMenuOpen ? <svg className="block h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
      ...
      {isMobileMenuOpen && <div className="sm:hidden" id="mobile-menu">
          <div className="pt-2 pb-3 space-y-1">
            <Link to="/courses" className="block pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium hover:bg-gray-50 hover:border-bitcoin transition">
              Cursos
            </Link>
          ...
          <div className="pt-4 pb-3 border-t border-gray-200">
            {isAuthenticated ? <>
                ...
                <div className="mt-3 space-y-1">
                  <Link to="/profile" className="block px-4 py-2 text-base font-medium hover:bg-gray-100">
                    Perfil
                  </Link>
                  <Link to="/dashboard" className="block px-4 py-2 text-base font-medium hover:bg-gray-100">
                    Panel de Control
                  </Link>
                  <Link to="/wallet" className="block px-4 py-2 text-base font-medium hover:bg-gray-100">
                    Monedero
                  </Link>
                  <button onClick={logout} className="block w-full text-left px-4 py-2 text-base font-medium hover:bg-gray-100">
                    Cerrar Sesión
                  </button>
                </div>
